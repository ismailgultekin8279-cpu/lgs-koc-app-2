...
for s in ismails:
    user_str = "NONE"
    try:
        if hasattr(s, 'user'): user_str = f"{s.user.username} (ID: {s.user.id})"
        elif hasattr(s, 'auth_user'): user_str = f"Linked to user?" # Handle different possible field names
    except: pass
    
    # Check for User -> Student link too
